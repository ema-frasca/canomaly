...
class MVTECADDecoder_old(nn.Module):
...
class MVTECADDecoder(Decoder):
    ...
    def __init__(self, code_length: int,
        ...
        super(MVTECADDecoder, self).__init__(code_length=code_length,
                                             output_shape=output_shape)

    def _set_conv_block(self):
        # Convolutional network
        self.conv = nn.Sequential(
            UpsampleBlock(channel_in=256*self.m, channel_out=128*self.m, activation_fn=self.activation_fn),
            UpsampleBlock(channel_in=128*self.m, channel_out=64*self.m, activation_fn=self.activation_fn),
            UpsampleBlock(channel_in=64*self.m, channel_out=32*self.m, activation_fn=self.activation_fn),
            ResidualBlock(channel_in=32*self.m, channel_out=32*self.m, activation_fn=self.activation_fn),
            nn.Conv2d(in_channels=32*self.m, out_channels=self.output_shape[0], kernel_size=1, bias=False)
        )
