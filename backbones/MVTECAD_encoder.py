...
class MVTECADEncoder_old(nn.Module):
...
class MVTECADEncoder(Encoder):
    ...
    def __init__(self, input_shape: Tuple[int, int, int], code_length: int, variational: bool = False,
        ...
        self.m = 1
        c, h, w = input_shape
        self.deepest_shape = (256*self.m, h // 8, w // 8)
        super(MVTECADEncoder, self).__init__(input_shape=input_shape, code_length=code_length, variational=variational,
    ...
    def _set_conv_block(self):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels=c, out_channels=32*self.m, kernel_size=3, bias=False),
            self.activation_fn,
            ResidualBlock(channel_in=32*self.m, channel_out=32*self.m, activation_fn=self.activation_fn),
            DownsampleBlock(channel_in=32*self.m, channel_out=64*self.m, activation_fn=self.activation_fn),
            DownsampleBlock(channel_in=64*self.m, channel_out=128*self.m, activation_fn=self.activation_fn),
            DownsampleBlock(channel_in=128*self.m, channel_out=256*self.m, activation_fn=self.activation_fn),
        )